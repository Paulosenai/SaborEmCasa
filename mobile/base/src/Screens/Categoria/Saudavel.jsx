...
export default function Home() {
  ...
  useEffect(() => {
    axios.get('http://10.0.2.2:8085/api/readNews/Saudavel/')
      .then(response => {
  ...
  const fetchData = useCallback(async () => {
    try {
      const response = await axios.get('http://10.0.2.2:8085/api/readNews/saudavel');
      ...
      <ScrollView onScroll={handleScroll} scrollEventThrottle={16}>  
        <View style={styles.section}>
          <View style={styles.newsCard}>
            <ImageBackground
              source={require("../../../res/img/saudavel.png")}
